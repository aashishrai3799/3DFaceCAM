...
class Test:
    def __init__(self,gan_model):
        ...
        self.template_verts = torch.from_numpy(get_template_verts()).to(device)
        self.face_v = torch.from_numpy(np.load('./data/face_v.npy')).to(dtype=torch.float32).to(device).unsqueeze(0) - 1

        self.z_noise = torch.randn(1,5).to(device)
        self.z_id = torch.randn(1,20).to(device)
        self.z_id_style = torch.randn(1,20).to(device)
        self.z_exp = torch.nn.functional.one_hot(torch.tensor([0]), num_classes=20).to(device)
    ...
    def generate_intensity(self, filename, low=0, high=1.5, num_steps=15, exp_list=range(0,20), save_obj=False, render=True):
        for i in exp_list:
            ...
            z_exp = torch.nn.functional.one_hot(torch.tensor([i]), num_classes=20).to(device)
...
if __name__ == '__main__':
    ...
    path_d = './ae/checkpoints/Dec/2'
    path_gid = os.path.join(gan.folder, 'Generator_Checkpoint_id/0.1')
    path_gexp = os.path.join(gan.folder, 'Generator_Checkpoint_exp/0.1')
