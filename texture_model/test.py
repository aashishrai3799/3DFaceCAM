...
def test(g_running, num_imgs=10, input_code_size=512, device='cuda:0', alpha=1.00, out_path='texture_results/'):
	...
	with open('assets/zid_dictionary.pkl', 'rb') as f:
	...
	for num in range(num_imgs):
		...
		id_path = os.path.join(out_path, 'sample_'+ str(num))
		...
		#for exp in [0,1,2,17]:  # For specific expressions
			...
			for intensity in range(20):
				...
				with torch.no_grad():
					...
					z_exp = torch.from_numpy(exp_enc_test)*0.05*intensity
					...
					print('Generating ID:', num, exp_enc_test, end='\r')
...
if __name__ == '__main__':
    ...
    test(g_running, num_imgs=2, input_code_size=input_code_size, device=device, alpha=1, out_path='texture_results/')
